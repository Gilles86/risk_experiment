...
def main(subject, session, bids_folder='/data'):
    ...
    wf = pe.Workflow(name=f'resample_{subject}_{session}', base_dir='/tmp')
    ...
    for par in parameters:
        ...
        inputnode.inputs.source_file = op.join(bids_folder, f'derivatives/encoding_model/sub-{subject}/ses-{session}/func/sub-{subject}_ses-{session}_desc-{par}.optim_space-T1w_pars.nii.gz')
