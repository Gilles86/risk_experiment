...
subjects = ['02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12', '13', '14', '32']

for subject, session, hemi, par in product(subjects, ['3t1', '7t1'], ['L', 'R'], parameters):
    fn = op.join(dir_.format(session=session), f'sub-{subject}_ses-{session}_desc-{par}_space-fsaverage_hemi-{hemi}.func.gii')
...
d['mu.optim.3t.natural'] = cortex.Vertex(np.exp(data.loc[('3t1', 'mu.optim')].mean(0)).values, 'fsaverage')
d['mu.optim.7t.natural'] = cortex.Vertex(np.exp(data.loc[('7t1', 'mu.optim')].mean(0)).values, 'fsaverage')
...
d['weighted_mu_3t_natural'] = cortex.Vertex(np.exp(get_weighted_mu(data.loc[['3t1']])).values, 'fsaverage',
        vmin=0, vmax=28)
d['weighted_mu_7t'] = cortex.Vertex(get_weighted_mu(data.loc[['7t1']]).values, 'fsaverage')
d['weighted_mu_7t_natural'] = cortex.Vertex(np.exp(get_weighted_mu(data.loc[['7t1']])).values, 'fsaverage',
        vmin=0, vmax=28)
