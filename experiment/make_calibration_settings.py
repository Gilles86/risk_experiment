...
def main(subject):
    ...
    for ix, p in enumerate(prob1):
        tmp = pd.DataFrame(product(base, fractions), columns=['base number', 'fraction'])
        tmp['p1'] = p
        tmp['p2'] = prob2[ix]
    ...
    df.loc[df['p1'] == 1.0, 'n2'] = (df['fraction'] * df['base number']).astype(int)
    df.loc[df['p1'] != 1.0, 'n1'] = (df['fraction'] * df['base number']).astype(int)
    ...
    df = df.loc[ixs].sort_index(level='run', sort_remaining=False).reset_index('p1')

    if not op.exists(op.abspath('settings')):
        os.makedirs(op.abspath('settings'))

    df['trial'] = np.arange(1, len(df)+1)
    df.to_csv(op.abspath(op.join('settings', f'sub-{subject}_ses-calibrate.tsv')), sep='\t')
